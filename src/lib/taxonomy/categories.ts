...
/**
 * Mapeo DIRECTO de CategoryId → Token RAW para queries del chatbot.
 * Este es el token EXACTO que aparece en las keys de Mindsaic (e.g., "fiestas y tradiciones").
 * Usado para construir patterns sin necesidad de llamar getCategorySearchPattern.
 * IMPORTANTE: Preservar caracteres especiales como ñ, tildes, etc. tal como aparecen en los datos.
 *
 * EXCEPCIONES:
 * - espaciosMuseisticos usa "espacios" (primera palabra) - requiere wildcard "espacios *"
 * - laRabida usa "la r" (inicio) - requiere wildcard "la r*"
 */
export const CHATBOT_CATEGORY_TOKENS: Record<CategoryId, string> = {
  naturaleza: "naturaleza",
  fiestasTradiciones: "fiestas y tradiciones",
  playas: "playas",
  espaciosMuseisticos: "espacios", // SOLO primera palabra, requiere wildcard
  ...
  laRabida: "la r", // SOLO inicio, requiere wildcard
  lugaresColombinos: "lugares colombinos",
  otros: "otros",
};

/**
 * Categorías que requieren wildcard (*) en sus patterns debido a variaciones en los datos.
 * Estas categorías usan solo parte del token en CHATBOT_CATEGORY_TOKENS.
 */
export const CHATBOT_CATEGORY_NEEDS_WILDCARD: Set<CategoryId> = new Set([
  "espaciosMuseisticos", // "espacios *" - variantes: museísticos, museiticos, etc.
  "laRabida", // "la r*" - variantes: rábida, rabida, etc.
]);
