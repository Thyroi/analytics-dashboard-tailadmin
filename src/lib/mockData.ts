...
function enumerateBuckets(
  ...
  const out: string[] = [];
  if (g === "d") {
    for (let cur = parseISO(startISO); cur <= parseISO(endISO); cur = addDays(cur, 1)) {
...
function deriveRangeFromQuery(
  ...
  if (g === "d") {
    const s = startTime
      ? `${startTime.slice(0, 4)}-${startTime.slice(4, 6)}-${startTime.slice(6, 8)}`
      : PROJECT_START_ISO;
    const e = endTime
  ...
  if (g === "m") {
    const sISO = startTime ? `${startTime.replace("/", "-")}-01` : PROJECT_START_ISO;
    const eISO = endTime
...
export function generateMockResponse(input: MindsaicInput): MindsaicOutput {
  ...
  for (const [tag, series] of allTagsSeries.entries()) {
    if (re.test(tag)) {
      ...
      const sorted = [...series].sort((a, b) => (a.time < b.time ? -1 : a.time > b.time ? 1 : 0));
      output[tag] = sorted;
    }
  }
