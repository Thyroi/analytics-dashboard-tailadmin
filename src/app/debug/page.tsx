...
export default function DebugPage() {
  ...
  const startDateStr = mode === "range" ? startDate.toISOString().split("T")[0] : undefined;
      ...
      <div className="p-6">
        ...
        {townHookResult.isLoading && (
          <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <p className="text-blue-700 dark:text-blue-300">🔄 Cargando datos...</p>
          </div>
        )}

        {townHookResult.isError && (
          <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
            <p className="text-red-700 dark:text-red-300">❌ Error: {String(townHookResult.error)}</p>
          </div>
        )}
        ...
        <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <div className="text-sm space-y-1">
            ...
            <p><strong>Start Date Str:</strong> {startDateStr || "undefined"}</p>
            <p><strong>End Date Str:</strong> {endDateStr}</p>
            <p><strong>Current End ISO:</strong> {currentEndISO}</p>
            <p className="text-xs text-blue-600 dark:text-blue-400 mt-2">
              ⚠️ Nota: En modo &quot;granularity&quot;, startDate se establece como undefined intencionalmente.
              La API debe calcular automáticamente el rango basado en granularity + endDate.
            </p>
          </div>
        </div>
        ...
        {/* Cards de Towns */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold mb-4">🏘️ Towns Data Cards ({townHookResult.data.length} towns):</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
            {townHookResult.data.map((town) => (
              <div key={town.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-300">GA4 Previous:</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-300">Chatbot Current:</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-300">Chatbot Previous:</span>
                  ...
                  {/* Combined Total */}
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-300 font-medium">Combined:</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-300">Delta %:</span>
                    <span className="font-semibold text-gray-500">
                      {town.combinedDeltaPct ? `${town.combinedDeltaPct.toFixed(1)}%` : 'N/A'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
          ...
          <div className="text-sm space-y-1">
            <p><strong>Granularidad:</strong> {granularity}</p>
            <p><strong>Start Date (input):</strong> {startDateStr || 'undefined'}</p>
            <p><strong>End Date (input):</strong> {endDateStr || 'undefined'}</p>
            <p><strong>Total Towns:</strong> {townHookResult.data.length}</p>
            <p><strong>Loading:</strong> {String(townHookResult.isLoading)}</p>
            <p><strong>Error:</strong> {String(townHookResult.isError)}</p>
            <p><strong>Towns with data:</strong> {townHookResult.data.filter(t => t.combinedTotal > 0).length}</p>
          </div>
        </div>

        {/* Estado completo del Hook */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold mb-4">🔍 Hook State Completo:</h3>
