...
import { computeRangesFromQuery, computeRangesForSeries } from "@/lib/utils/time/timeWindows";
...
describe("Granularity Ranges Debug", () => {
  ...
  it("should verify granularity logic - behavior without params (useResumen hooks)", () => {
    const testCases = [
      { granularity: "d" as Granularity, expected: { currentDays: 7, previousDays: 7 } }, // Para series debe ser 7
      { granularity: "w" as Granularity, expected: { currentDays: 7, previousDays: 7 } },
      { granularity: "m" as Granularity, expected: { currentDays: 30, previousDays: 30 } }, // aproximado
    ];
    ...
    testCases.forEach(({ granularity, expected }) => {
      ...
      const currentDays = getDaysBetween(ranges.current.start, ranges.current.end);
      const previousDays = getDaysBetween(ranges.previous.start, ranges.previous.end);
      
      if (granularity !== "m") { // El mes puede variar
  ...
  it("should ensure week includes 7 days and previous week is shifted correctly", () => {
    ...
    expect(previousStart.toISOString().split('T')[0]).toBe(expectedPrevStart.toISOString().split('T')[0]);
    expect(previousEnd.toISOString().split('T')[0]).toBe(expectedPrevEnd.toISOString().split('T')[0]);
  });
});
