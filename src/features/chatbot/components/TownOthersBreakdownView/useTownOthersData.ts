...
import type {
  TownOthersDataResult,
  UseTownOthersDataParams,
} from "./TownOthersBreakdownView.types";
 ...
 * Version town-first (context de pueblo)
 */
export function useTownOthersData({
  othersBreakdown,
  granularity,
}: UseTownOthersDataParams): TownOthersDataResult {
  return useMemo(() => {
    ...
    for (const entry of othersBreakdown) {
      ...
      let data = byLeaf.get(leaf);
      if (!data) {
        data = { total: 0, timePoints: new Map() };
        byLeaf.set(leaf, data);
      }
      data.total += entry.value;
      ...
      for (const point of entry.timePoints || []) {
        ...
        const iso =
          timeStr.length === 8
            ...
            : timeStr;
        const prev = data.timePoints.get(iso) || 0;
        data.timePoints.set(iso, prev + point.value);
      }
    }
